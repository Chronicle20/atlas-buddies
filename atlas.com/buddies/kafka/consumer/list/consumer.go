...
func InitHandlers(l logrus.FieldLogger) func(db *gorm.DB) func(rf func(topic string, handler handler.Handler) (string, error)) {
	return func(db *gorm.DB) func(rf func(topic string, handler handler.Handler) (string, error)) {
		return func(rf func(topic string, handler handler.Handler) (string, error)) {
			...
			_, _ = rf(t, message.AdaptHandler(message.PersistentConfig(handleUpdateCapacityCommand(db))))
		}
	}
}
...
func handleUpdateCapacityCommand(db *gorm.DB) message.Handler[list2.Command[list2.UpdateCapacityCommandBody]] {
	return func(l logrus.FieldLogger, ctx context.Context, c list2.Command[list2.UpdateCapacityCommandBody]) {
		if c.Type != list2.CommandTypeUpdateCapacity {
			return
		}
		err := list.NewProcessor(l, ctx, db).UpdateCapacityAndEmit(c.CharacterId, c.WorldId, c.Body.Capacity)
		if err != nil {
			l.WithError(err).Errorf("Error updating capacity to [%d] for character [%d] buddy list.", c.Body.Capacity, c.CharacterId)
		}
	}
}
